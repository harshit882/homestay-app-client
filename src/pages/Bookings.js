...
const BookingsPage = () => {
  ...
  const setPlaces =()=> {
    fetch(`https://homestay-app-server.azurewebsites.net/bookings`)
    .then((data)=>{
  ...
  const handelDeleteBookedPlace =(id)=>{
    console.log(id)
    fetch(`https://homestay-app-server.azurewebsites.net/bookings/${id}`,{
      method: 'DELETE'
    })
    .then((data)=>{
      ...
      <div className="">
        {bookings?.length > 0 &&
          bookings.map((booking) => (
            ...
            <button className="absolute bottom-6 bg-red-400 text-white px-4 py-2 rounded-md right-10 hover:bg-red-500 flex flex-row gap-1 items-center justify-center" onClick={()=>handelDeleteBookedPlace(booking._id)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
